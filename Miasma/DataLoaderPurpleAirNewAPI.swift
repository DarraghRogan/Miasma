...
struct PurpleAirNewAPIDataStructure: Codable {
//    var mapVersion, baseVersion, mapVersionString: String?
    var results: [PurpleAirResult]?
}

struct PurpleAirNewAPIResult: Codable {
    var id: Int
    var label: String
...
var purpleAirNewAPIData = PurpleAirNewAPIDataStructure()

 public class DataLoaderPurpleAirNewAPI {

    
    func loadPurpleAirNewAPIData(id:String) {
        ...
        let request = NSMutableURLRequest(url: NSURL(string:
         "https://api.purpleair.com/v1/sensors/\(id)?fields=pm2.5_10minute")! as URL,
        ...
        let dataTask = session.dataTask(with: request as URLRequest, completionHandler: { (data, response, error) -> Void in
            if (error != nil) {
                ...
                print("Received from the Purple Air New API")
                if let data = data,
                ...
                do {
                    let dataFromPurpleAirNewAPI = try decoder.decode(PurpleAirNewAPIDataStructure.self, from: data!)
                    purpleAirNewAPIData = dataFromPurpleAirNewAPI
